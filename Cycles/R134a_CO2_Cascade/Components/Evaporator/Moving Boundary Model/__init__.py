...
class HeatExchanger(object):
    ...
    Dh = 0.0048  # hydraulic diameter [m]
    S = 0.000994  # Cross sectional area per channel [m2]
    k = 16.3
    ...
    def pressure_loss(self, p, h, T, Tw, m, phase, fluid, A_req):
        ...
        if phase == 'two-phase':
            ...
            if fluid == 'R134a':
                ...
                q = (lamda_l * (1 - x) + lamda_v * x) * np.abs(T - Tw) / t
                Bo = q / (G * h_evap)
                f = 94.75 * Re_eq ** (-0.0467) * Re ** (-0.4) * Bo ** (0.5) * (p / p_crit) ** (0.8)
        ...
        if component.linearized:
            ...
            for port in component.ports:
                if port.port_id[2] == psd['-h']:
                    port.p.set_value(component.lamda * p_out_h + (1 - component.lamda) * F[3])
                    port.h.set_value(component.lamda * h_out_h + (1 - component.lamda) * F[4])
                    port.m.set_value(component.lamda * m_in_h + (1 - component.lamda) * F[5])

                elif port.port_id[2] == psd['-c']:
                    port.p.set_value(component.lamda * p_out_c + (1 - component.lamda) * F[0])
                    port.h.set_value(component.lamda * h_out_c + (1 - component.lamda) * F[1])
                    port.m.set_value(component.lamda * m_in_c + (1 - component.lamda) * F[2])
        else:
            for port in component.ports:
                if port.port_id[2] == psd['-c']:
